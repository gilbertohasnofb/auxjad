...
class Drifter():
    ...
    Basic usage:
        ...
        the previous results. That is, either a note head of the first
        container is removed or a note head of the second container is added.
    ...
    Arguments and properties:
        ...
        note head from the second container must be faded in before any note
        heads of the first container are removed (default ``False``). Setting
        :attr:`fade_out_last` to ``True`` will ensure that all note heads from
        the second container must be faded in before the very last note head of
        the first container is removed (default ``False``). The arguments
        ...
        note heads as well its length), making it more likely that both will
        fade in and out at a similar rate (default value is ``False``).
        :attr:`disable_rewrite_meter` disables
        the |abjad.mutate().rewrite_meter()| mutation which is applied
        to the container after every call, and :attr:`omit_time_signatures`
        will remove all time signatures from the output (both are ``False`` by
    ...
    :func:`len()`:
        ...
        of both :attr:`contents_in` and :attr:`contents_out`.
    ...
    :attr:`fade_in_first` and :attr:`fade_out_last`:
        ...
        a note head from second container must be faded in before any note head
        of the first container is removed (default ``False``). Setting
        :attr:`fade_out_last` to ``True`` will ensure that all note heads from
        the second container must be faded in before the very last note head of
        first container is removed (default ``False``).
    ...
    :attr:`weighted_duration`:
        ...
        the larger container (considering both its number of note heads as well
    ...
    def __len__(self) -> int:
        r'Returns the sum of the number of note heads of both contents.'
    ...
    def weighted_duration(self) -> bool:
        r"""Weights the choice of fader according to its number of note heads
        and total duration.
        """

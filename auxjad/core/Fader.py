...
class Fader():
    ...
    Basic usage:
        ...
        (that is, its note heads will be gradually removed one by one).
    ...
    :attr:`fader_type`:
        ...
        original note heads one by one.
    ...
    Arguments and properties:
        ...
        maximum number of note heads that can be faded in/out at each
        iteration, ranging between ``1`` and the input value (default is also
        ``1``). By default, calling the object in fade out mode will return the
        original container, and calling it in fade in mode will return a
        container filled with rests; set :attr:`process_on_first_call` to
        ``True`` and the fade process will be applied on the very first call.
        ...
        to disable this behaviour. An initial mask for the note heads can be
        set using :attr:`mask`, which should be a :obj:`list` of the same
        length as the number of note heads in the input container. When
        ...
        with some specific note heads already hidden/shown. The properties
    ...
    :func:`len()`:
        ...
        Note that each individual note head in a chord will count as one note
        head.
    ...
    :attr:`mask` and :meth:`reset_mask`:
        The property :attr:`mask` is used to represent whether each note heads
        is hidden or shown. It is a :obj:`list` of the same length as the
        number of note heads in the input container (use the :func:`len()`
        function to read that value). When :attr:`fader_type` is set to
        ``'out'``, the mask is initialised with ``1``'s, and when it is set to
        ``'in'``, it is initialised with ``0``'s. Change it to a mix of ``1``'s
        and ``0``'s to start the process with some note heads already
        hidden/shown. Use the method :meth:`reset_mask` to reset it back to its
        default value (depending on :attr:`fader_type`).
        ...
        highest pitched one. For instance, the container ``c'2 <d' e' f' g'>2``
        has five note heads, and applying the mask ``[1, 0, 1, 1, 0]`` to it
        will result in the first, third, and fourth note heads to be shown, in
        this case ``c'2 <e' f'>2``.
    ...
    :meth:`shuffle_mask`:
        ...
        shuffle the mask while keeping the total number of shown note heads
        constant (that is, it will shuffle the contents of the mask while
        keeping the total number of ``1``'s and ``0``'s).
    ...
    def __len__(self) -> int:
        r'Returns the number of note heads of :attr:`contents`.'
        length = 0
    ...
    def reset_mask(self):
        r'Creates a mask filled with a default value for the note heads.'
        self._is_first_window = True
        if self._fader_type == 'out':
        ...
        for logical_tie in logical_ties:
            if (isinstance(logical_tie.head, abjad.Chord)
                ...
                if 1 not in self._mask[mask_index:mask_index + chord_len]:
                    ...
                    new_note_heads = []
                    for note_head in logical_tie.head.note_heads:
                        if self._mask[mask_index] != 0:
                            new_note_heads.append(note_head)
                        mask_index += 1
                    if len(new_note_heads) > 1:
                        for leaf in logical_tie:
                            leaf.note_heads = new_note_heads
                    else:
                        for leaf in logical_tie:
                            note = abjad.Note(new_note_heads[0],
    ...
    @property
    def mask(self) -> list:
        r"""Mask with ``1``'s and ``0``'s representing the note heads of
        :attr:`contents`.
        """
        return self._mask

...
class Fader():
    ...
    Basic usage:
        ...
        out (that is, its note heads will be gradually removed one by one).
    ...
    Changing ``fader_type`` after initialisation:
    ...
    Arguments and properties:
        ...
        of note heads that can be faded in/out at each iteration, ranging
        ...
        An initial mask for the note heads can be set using ``mask``, which
        should be a ``list`` of the same length as the number of note heads in
        the input container. When ``fader_type`` is set to ``'out'``, the mask
        is initialised with ``1``'s, and when it is set to ``'in'``, it is
        initialised with ``0``'s. Change it to a mix of ``1``'s and ``0``'s to
        start the process with some specific note heads already hidden/shown.
    ...
    ``mask`` and ``reset_mask()``:
        ...
        method ``reset_mask()`` to reset it back to its default value
        (depending on ``fader_type``).
    ...
    def __len__(self) -> int:
        r'Returns the number of note heads of ``contents``.'
        length = 0

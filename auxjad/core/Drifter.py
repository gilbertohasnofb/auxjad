...
class Drifter():
    ...
    Basic usage:
        ...
        the previous results. That is, either a note head of ``container_out``
        is removed or a note head of ``container_in`` is added.
    ...
    ``output_n()``:
        ...
        of two |abjad.Staff|'s, use the method ``output_n()`` and pass the
    ...
    Arguments and properties:
        ...
        note head from ``container_in`` must be faded in before any note heads
        of ``container_out`` are removed (default ``False``). Setting
        ``fade_out_last`` to ``True`` will ensure that all note heads from
        ``container_in`` must be faded in before the very last note head of
        ...
        larger container (considering both its number of note heads as well its
        ...
        disable this behaviour. The properties ``boundary_depth``,
    ...
    ``fade_in_first`` and ``fade_out_last``:
        Setting the property ``fade_in_first`` to ``True`` will ensure that a
        note head from ``container_in`` must be faded in before any note head
        of ``container_out`` are removed (default ``False``). Setting
        ``fade_out_last`` to ``True`` will ensure that all note heads from
        ``container_in`` must be faded in before the very last note head of
    ...
    ``initial_repetitions`` and ``final_repetitions``:
    ...
    Tweaking or disabling |abjad.mutate().rewrite_meter()|:
        ...
        |abjad.mutate().rewrite_meter()| are ``boundary_depth``,
        ``maximum_dot_count`` and ``rewrite_tuplets``, which work exactly as
        the identically named arguments of |abjad.mutate().rewrite_meter()|.

    ``contents_in`` and ``contents_out``:
        Containers can be switched after initialisation by overwriting the
        properties ``contents_in`` or ``contents_out``. This will reset the
        process.

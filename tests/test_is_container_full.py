...
def test_is_container_full_01():
    ...
    assert auxjad.is_container_full(container1)
    assert not auxjad.is_container_full(container2)
    assert not auxjad.is_container_full(container3)
    assert auxjad.is_container_full(container4)


def test_is_container_full_02():
    ...
    assert auxjad.is_container_full(container1)
    assert auxjad.is_container_full(container2)
    assert not auxjad.is_container_full(container3)
    assert not auxjad.is_container_full(container4)


def test_is_container_full_03():
    container = abjad.Container(r"c'4 d'4 e'4 f'4")
    time_signature = abjad.TimeSignature((3, 4), partial=(1, 4))
    abjad.attach(time_signature, container[0])
    assert auxjad.is_container_full(container)


def test_is_container_full_04():

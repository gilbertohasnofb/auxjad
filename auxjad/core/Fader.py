...
class Fader():
    ...
    Arguments and properties:
        ...
        to the container after every call, and ``omit_time_signatures`` will
        ...
        ``1``'s and ``0``'s to start the process with some specific note heads
        already hidden/shown. The properties ``boundary_depth``,
        ``maximum_dot_count``, and ``rewrite_tuplets`` are passed as arguments
        to |abjad.mutate().rewrite_meter()|, see its documentation for more
        information.

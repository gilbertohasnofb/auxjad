...
class DeBruijnGenerator:
    ...
    implementation by Joseph Sawada, 2015-2018. This class can be used to generate sequences of
    elements from an input :obj:`list` in which every combination of :math:`n` elements from an
    alphabet of size :math:`k` are present.
    ...
    _DE_BRUIJN_ALGORITHMS = ["granddaddy", "grandmama"]
    ...
    def _generate_sequence(self) -> None:
        ...
        while True:
            ...
            next_sym = successor_generator(window, self._order, self._alphabet_size)
            window = window[1:] + [next_sym]
    ...
    @staticmethod
    def _granddaddy_generator(window: list[int], order: int, alphabet_size: int) -> int:
        r"""Docstring"""  # TODO write docstring + code
        max_symbol = alphabet_size - 1
        next_symbol = DeBruijnGenerator._get_next_symbol(window, order, alphabet_size)
    ...
    @staticmethod
    def _get_next_symbol(window: list[int], order: int, alphabet_size: int) -> int | None:  # TODO review docstring + code
        r"""
        Compute the smallest valid next symbol for the GrandDaddy rule.

        Finds the tail of the window (all elements after the leading run of max symbols, starting at
        index 1), then uses its necklace period to determine the smallest symbol that keeps the sequence
        ...
        Returns:
            int | None: The smallest valid next symbol, or None if no valid symbol exists (the caller
                should keep the current symbol).
        """
        max_symbol = alphabet_size - 1
        ...
        # Pad with a leading zero so that period arithmetic (tail[i - period]) uses 1-based positions,
        # keeping i and period directly comparable
        tail_length = order - first_non_max
        tail = [0] + list(window[first_non_max:]) + [0]
        ...
        next_symbol = tail[tail_length + 1] = tail[(tail_length + 1) - period]
    ...
    @staticmethod
    def _grandmama_generator(window: list[int], order: int, alphabet_size: int) -> int:
        r"""Docstring"""  # TODO write docstring + code
        x = DeBruijnGenerator._get_grandmama_x(window, order, alphabet_size)

        if x != 0 and window[0] == x:
            return 0
        if x != 0 and window[0] < x:
            return window[0] + 1
        return window[0]
    ...
    @staticmethod
    def _get_grandmama_x(window: list[int], order: int, alphabet_size: int) -> int:  # TODO review docstring + code
        r"""
        Compute the largest valid candidate symbol for the GrandMama rule.

        Scans backward from the end of the window, skipping trailing min-symbols (zeros), to find
        the length ``j`` of the meaningful suffix.  Builds the rotated candidate string
        ``0^{order-j} · window[0..j-1]``, then returns the largest ``x`` in
        ``{1, ..., alphabet_size - 1}`` such that substituting ``x`` at position ``order - j``
        produces a necklace.

        GrandMama requires trying multiple values of ``x`` against the same base string, which is
        why it calls ``_is_necklace`` rather than inlining the period logic as GrandDaddy does.
        ...
        Returns:
            int: The largest valid x, or 0 if no such value exists.
        """
        # Scan from the end of the window backward while symbols equal 0 (the min symbol).
        # j is a 1-based count of the meaningful prefix length; window[j-1] is the last non-zero
        # symbol (or j stays at 1 if the entire window is zero).
        j = order
        while j > 1 and window[j - 1] == 0:
            j -= 1

        # Build the rotated candidate string b of length order:
        #   (order - j) leading zeros, then window[0..j-1]
        b = [0] * (order - j) + list(window[:j])

        # Try x from the largest possible symbol down to 1; return the first that yields a necklace.
        # b[order - j] is the insertion point (the position of x in the rotated string).
        for x in range(alphabet_size - 1, 0, -1):
            b[order - j] = x
            if DeBruijnGenerator._is_necklace(b):
                return x
        return 0

    @staticmethod
    def _is_necklace(sequence: list[int]) -> bool:  # TODO review docstring + code
        r"""
        Return True iff ``sequence`` is a necklace (its own lexicographically minimal rotation).

        Pads with a leading zero so that period arithmetic uses 1-based positions, keeping ``i``
        and ``p`` directly comparable — the same convention used by ``_get_next_symbol``.
        ...
        """
        n = len(sequence)
        # Pad with a leading zero so that period arithmetic (b[i - p]) uses 1-based positions,
        # keeping i and p directly comparable
        b = [0] + sequence
        p = 1
    ...
    def algorithm(self) -> str:
        r"""The :obj:`str` with the name of the algorithm. Options include ``"granddaddy"`` or
        ``"grandmama"``.
        """
